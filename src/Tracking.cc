...
bool Tracking::NeedNewKeyFrame()
{
    ...
    const bool cTimeGap = mbUseIMU && ((mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= timegap) && bLocalMappingIdle;
...
bool Tracking::TrackWithSparseAlignment(bool bTrackLastKF)
{
    ...
    for (int i = 0; i < mLastFrame.N; i++)
        if (mLastFrame.mvpMapPoints[i] && mLastFrame.mvpMapPoints[i]->isBad() == false && mLastFrame.mvbOutlier[i] == false)
...
{
    ScopedTrace st("SearchLocalPointD");
    int cntSuccess = 0;
